...
from app.main.forms import PostForm,EmptyForm,EditForm
...
def index():
    ...
    posts=current_user.followed_posts().paginate(page=pages,per_page=current_app.config['POSTS_PER_PAGE'],error_out=False)
...
def explore():
    pages=request.args.get('page',1,type=int)
    posts=Post.query.order_by(Post.timestamp.desc()).paginate(page=pages,per_page=current_app.config['POSTS_PER_PAGE'],error_out=False)
